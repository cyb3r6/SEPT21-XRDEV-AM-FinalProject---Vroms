...
public class GrabbableInTrigger : MonoBehaviour
{
    ...
    private void Update()
    {
        UpdateClosetsGrabbable();
    }

    void UpdateClosetsGrabbable()
    {
        nearbyGrabbables = CleanedGrabbables(nearbyGrabbables);
    ...
    private Dictionary<Collider, XRGrabInteractable> GetValidGrabbables(Dictionary<Collider, XRGrabInteractable> grabs)
    {
        _valids = new Dictionary<Collider, XRGrabInteractable>();
        if(grabs == null)
        {
            return _valids;
        }

        // check for objects that need to be removed from remote grabbabesl
