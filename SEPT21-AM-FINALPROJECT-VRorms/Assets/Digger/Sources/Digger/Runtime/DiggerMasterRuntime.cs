...
namespace Digger
{
    ...
    public class DiggerMasterRuntime : MonoBehaviour
    {
        ...
        public IEnumerator ModifyAsync(Vector3 position, BrushType brush, ActionType action, int textureIndex, float opacity,
            float size, bool removeDetails = true, bool removeTreesInSphere = true, float stalagmiteHeight = 8f,
            bool stalagmiteUpsideDown = false, bool opacityIsTarget = false, Action callback = null)
        {
            if (isRunningAsync) {
                Debug.LogError("Cannot Modify as asynchronous modification is already in progress");
                yield break;
        ...
        /// <see cref="ModifyAsync(UnityEngine.Vector3,Digger.BrushType,Digger.ActionType,int,float,float,bool,bool,float,bool,bool,System.Action)"/>
        /// <param name="p">Modification parameters</param>
        public IEnumerator ModifyAsync(ModificationParameters p)
        {
            return ModifyAsync(p.Position, p.Brush, p.Action, p.TextureIndex, p.Opacity, p.Size, p.RemoveDetails, p.RemoveTreesInSphere, p.StalagmiteHeight, p.StalagmiteUpsideDown, p.OpacityIsTarget, p.Callback);
        }
        ...
        /// <see cref="ModifyAsyncBuffured(UnityEngine.Vector3,Digger.BrushType,Digger.ActionType,int,float,float,bool,bool,float,bool,bool,System.Action)"/>
